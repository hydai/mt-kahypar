...
private:
  ...
  std::pair<PartitionID, HyperedgeWeight> computeBestTargetBlock(const PHG& phg,
    ...
    const auto [move_from_benefit, bitset] = phg.moveFromBenefitWithBitset(u);
    ...
    for (PartitionID i = 0; i < phg.k(); ++i) {
      if (i != from) {
        ...
        const HyperedgeWeight penalty = phg.moveToPenaltyWithBitset(u, i, bitset);
    ...
    const Gain gain = to != kInvalidPartition ? move_from_benefit - to_penalty
